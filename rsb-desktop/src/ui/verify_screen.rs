    ...
    let handle_verify = move |_| {
        ...
        spawn(async move {
            ...
            let progress_cb = Arc::new(move |cur, tot, msg: String| {
                let prog = if tot > 0 { cur as f64 / tot as f64 } else { 0.0 };
            ...
            let res = async {
                if bkp.as_os_str().is_empty() && s3_bucket_opt.is_none() { return Err(texts.define_backup_or_s3.to_string()); }
                ...
                core::perform_verify_with_cancellation(&cfg, None, false, is_fast, Some(progress_cb), Some(token)).await.map_err(|e| e.to_string())
            }.await;

            match res {
                Ok(mut report) => {
                    let _ = record_verify_operation(report.status.clone(), report.files_processed, report.files_with_errors, report.duration.as_secs(), bkp.to_string_lossy().to_string());
                    if create_report {
                        report.profile_path = "Desktop UI".to_string();
                        let html = rsb_core::report::generate_html(&report);
                        let filename = format!("rsb-report-verify-{}.html", Local::now().format("%Y%m%d-%H%M%S"));
                ...
                Err(e) => {
                    let _ = record_verify_operation("Falha".to_string(), 0, 0, 0, bkp.to_string_lossy().to_string());
