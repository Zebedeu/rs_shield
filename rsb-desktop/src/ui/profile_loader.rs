...
pub fn load_profile(path: &Path) -> Result<Config, String> {
    ...
    let mut config: Config = toml::from_str(&content)
        .map_err(|e| format!("Erro ao parsear TOML: {}", e))?;
...
impl From<Config> for ProfileData {
    fn from(cfg: Config) -> Self {
        let s3_bucket = cfg.s3.as_ref().and_then(|s| s.bucket.clone()).or(cfg.s3_bucket).unwrap_or_default();
        let s3_region = cfg.s3.as_ref().and_then(|s| s.region.clone()).or(cfg.s3_region).unwrap_or_default();
        let s3_endpoint = cfg.s3.as_ref().and_then(|s| s.endpoint.clone()).or(cfg.s3_endpoint).unwrap_or_default();
        let s3_access_key = cfg.s3.as_ref().and_then(|s| s.access_key.clone()).unwrap_or_default();
        let s3_secret_key = cfg.s3.as_ref().and_then(|s| s.secret_key.clone()).unwrap_or_default();
        ...
        ProfileData {
            ...
            s3_access_key,
            s3_secret_key,
            encrypt_patterns: cfg.encrypt_patterns.map(|v| v.join("\n")).unwrap_or_default(),
            pause_on_low_battery: cfg.pause_on_low_battery.map(|v| v.to_string()).unwrap_or_default(),
            pause_on_high_cpu: cfg.pause_on_high_cpu.map(|v| v.to_string()).unwrap_or_default(),
            compression_level: cfg.compression_level.map(|v| v.to_string()).unwrap_or_default(),
        }
    }
}
