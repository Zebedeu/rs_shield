    ...
    let handle_restore = move |_| {
        ...
        spawn(async move {
            ...
            let res = async {
                ...
                let excludes: Vec<String> = excludes_str.lines().map(|s| s.trim().to_string()).filter(|s| !s.is_empty()).collect();
                ...
                let target = if rst.as_os_str().is_empty() { None } else { Some(rst) };
                let token = cancellation_token();
                core::perform_restore_with_cancellation(&cfg, snap_opt.as_deref(), target, key_opt.as_deref(), true, Some(progress_cb), Some(token)).await.map_err(|e| e.to_string())
            }.await;

            match res {
                Ok(mut report) => {
                    let _ = record_restore_operation(report.status.clone(), report.files_processed, report.duration.as_secs(), bkp.to_string_lossy().to_string(), rst_for_record.to_string_lossy().to_string());
                    if create_report {
                        report.profile_path = "Desktop UI".to_string();
                        let html = rsb_core::report::generate_html(&report);
                        let filename = format!("rsb-report-restore-{}.html", Local::now().format("%Y%m%d-%H%M%S"));
                ...
                Err(e) => {
                    let _ = record_restore_operation("Falha".to_string(), 0, 0, bkp.to_string_lossy().to_string(), rst_for_record.to_string_lossy().to_string());
