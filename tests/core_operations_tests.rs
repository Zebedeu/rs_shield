...
fn test_backup_file_filtering() {
    ...
    for (file, should_include) in test_files {
        ...
        assert_eq!(!excluded, should_include, "File {} filtering mismatch", file);
    }
}
...
fn test_restore_snapshot_selection() {
    ...
    if let Some(id) = selected_id {
        assert!(available_snapshots.contains(&id), "Selected snapshot should exist");
    }
}
...
fn test_progress_callback() {
    ...
    for i in 1..=total_files {
        ...
        assert!(percentage >= 0.0 && percentage <= 100.0, "Percentage should be valid");
    }
...
fn test_file_priority_calculation() {
    ...
    let files = vec![
        FilePriority { path: "small.txt", size: 1024 },
        FilePriority { path: "medium.zip", size: 1024 * 1024 },
        FilePriority { path: "large.iso", size: 1024 * 1024 * 1024 },
    ];
    
    let mut sorted = files.clone();
    sorted.sort_by_key(|f| f.size);
    
    assert_eq!(sorted[0].path, "small.txt");
    assert_eq!(sorted[sorted.len()-1].path, "large.iso");
}
...
fn test_operation_timing() {
    ...
    assert!(duration.as_millis() >= 50, "Duration should be at least 50ms");
}
