...
use rsb_core::realtime::{
    FileChange, ChangeType, ChangeQueue, RealtimeSync, SyncStrategy, 
    sync_all_files
};
use chrono::Utc;
...
async fn test_sync_all_files_nested_directories() {
    ...
    fs::write(src_dir.path().join("subdir/nested/file.txt"), "nested").expect("Failed to write nested file");
...
async fn test_process_batch_filters_correctly() {
    ...
    sync.queue.add_change(FileChange {
        path: file1,
        change_type: ChangeType::Created,
        timestamp: Utc::now(),
        size: 9,
    ...
    assert_eq!(processed.len(), 1);
    assert!(processed[0].path.to_string_lossy().contains("important.txt"));
}
